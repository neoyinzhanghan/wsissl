...
# MOVE THE SUBFOLDERS INTO THE TRAIN, VAL, TEST SUBDIRECTORIES
# if symbolic is True, then create symbolic links instead of copying

for pdr in tqdm(train_pdrs, desc="Copying train folders"):
    if args.symbolic:
        os.symlink(pdr, os.path.join(args.save_dir, "train", os.path.basename(pdr)))
    else:  # use shutil
        shutil.copytree(
            pdr, os.path.join(args.save_dir, "train", os.path.basename(pdr))
        )

for pdr in tqdm(val_pdrs, desc="Copying val folders"):
    if args.symbolic:
        os.symlink(pdr, os.path.join(args.save_dir, "val", os.path.basename(pdr)))
    else:  # use shutil
        shutil.copytree(pdr, os.path.join(args.save_dir, "val", os.path.basename(pdr)))

for pdr in tqdm(test_pdrs, desc="Copying test folders"):
    if args.symbolic:
        os.symlink(pdr, os.path.join(args.save_dir, "test", os.path.basename(pdr)))
    else:  # use shutil
        shutil.copytree(pdr, os.path.join(args.save_dir, "test", os.path.basename(pdr)))
